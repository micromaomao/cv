...
\begin{document}
  ...
  \begin{itemize}
    ...
    \item Fluent: Python, Go, C/C++, Rust, JavaScript/TypeScript, Java. Quick to pick up new languages.
    \item Experience with cloud-native development on Azure and familiar with Google Cloud and AWS.
    ...
    \item Experience with full-stack Web development: React, Svelte (NextJS, SvelteKit), FastAPI, PostgreSQL, Redis.
  ...
  \begin{itemize}
    ...
    % {%
    %   \centering\begin{tikzpicture}[color=black!50!white, overlay]
    %     \draw[dashed] (0.0\textwidth,0) -- (0.8\textwidth,0pt) node[pos=0, fill=white, anchor=west, yshift=0.5pt] {Azure Core \hspace*{10pt}};
    %     \useasboundingbox (0,0) rectangle (\textwidth,0);
    %   \end{tikzpicture}%
    % }\vspace{2pt plus 4pt}%

    \begin{itemize}
      \item \textbf{Confidential Computing, Azure Core}: Working on Confidential Containers on Linux and Windows, making foundational Azure infrastructure more secure.
      \item Resolved issues relating to interrupt handling and devices for a Linux SEV-SNP guest under Hyper-V.
      % \item Created test suites to provoke problems, and CI pipelines to monitor regressions.
    % \end{itemize}\vspace{-2pt plus 2pt}
    %
    % {%
    %   \centering\begin{tikzpicture}[color=black!50!white, overlay]
    %     \draw[dashed] (0.0\textwidth,0) -- (0.8\textwidth,0pt) node[pos=0, fill=white, anchor=west, yshift=0.5pt] {Azure for Operators \hspace*{10pt}};
    %     \useasboundingbox (0,0) rectangle (\textwidth,0);
    %   \end{tikzpicture}%
    % }\vspace{2pt plus 4pt}%
    %
