\documentclass[a4paper]{article}
\usepackage[utf8x]{inputenc}
\usepackage{microtype}
\usepackage[english]{babel}
\usepackage[calc,showdow,english]{datetime2}
\usepackage[margin=0.8cm]{geometry}
\usepackage[T1]{fontenc}
\usepackage{helvet}
\renewcommand{\familydefault}{\sfdefault}
\usepackage{tikz}
\usepackage{graphicx}
\usepackage{enumitem}
\usepackage{xcolor}
\usepackage{textcomp}
\usepackage{amsmath}
\usepackage{mathtools}
\usepackage{transparent}
\usepackage{setspace}
\usetikzlibrary{calc}
\usetikzlibrary{positioning}
\graphicspath{{icons/}}
\input{icons.tex}

\makeatletter
\DeclareRobustCommand\tlabel{%
  \tikz[overlay, remember picture, baseline=(nlabel.base)]{\node[anchor=base, inner sep=0pt] (nlabel) {};}%
}
\DeclareRobustCommand\posttlabel{%
  \tikz[overlay, remember picture]{
    \coordinate (ll) at (nlabel.south -| current page.north west);
    \path[fill=link] ($(ll)-(0.1cm,0pt)$) rectangle ++(0.2cm, 0.9em);
  }%
}
\renewcommand\section[1]{%
  % \@startsection name level indent beforeskip afterskip style
  \@startsection{section}{1}{\z@}{0.1\baselineskip plus 0.5em}{0.1\baselineskip}{\bfseries\MakeUppercase}*{\tlabel{}#1\makebox[0pt]{\posttlabel}}%
}
\makeatother

\def\dashdiv{{%
  \def\sp{0.3em minus 0.1em}%
  \hspace{\sp}%
  \color{black!50!white}%
  --%
  \hspace{\sp}%
}}

\setlength{\parindent}{0em}
\setlength{\parskip}{1em plus 0.2em minus 0.5em}
\setstretch{1.05}

\setitemize{topsep=0em, itemsep=0.2\parskip, partopsep=0em, parsep=0.5\parskip}
\setlist{leftmargin=1.3em}

\colorlet{link}{blue!50!black}

\makeatletter
\DTMnewdatestyle{my}{%
  \renewcommand{\DTMDisplaydate}[4]{\DTMshortmonthname{##2} ##1}%
  \renewcommand{\DTMdisplaydate}{\DTMDisplaydate}%
}
\DTMsetdatestyle{my}
\usepackage[hidelinks, pdftex, pdfauthor={Tingmao Wang}, pdftitle={Tingmao Wang - \today}]{hyperref}
\makeatother

\pagestyle{empty}%

\def\ghurl#1{%
  \href{#1}{({\blueghicon{}})}%
}
